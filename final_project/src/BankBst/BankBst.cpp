...
void isMatch(const struct bst_node *node, const string& wild, const string& last)
{
	if(node == NULL)
		return;
	if(node->bst_link[0] != NULL)
		isMatch(node->bst_link[0], wild, last);
	if(wildcmp(wild, ((Account*)node->bst_data)->id) && ((Account*)node->bst_data)->id != last){
		if(!check_find){
			check_find = true;
			cout << ((Account*)node->bst_data)->id;
		}
		else
			cout << "," << ((Account*)node->bst_data)->id;
	}
	if(node->bst_link[1] != NULL)
		isMatch(node->bst_link[1], wild, last);
}

void traverse(const struct bst_node *node, vector <pair <string, int> >& recommend, const string& id)
{
	if(node == NULL)
		return;
	recommend.push_back(make_pair(((Account*)node->bst_data)->id, score(((Account*)node->bst_data)->id, id)));
	if(node->bst_link[0] != NULL || node->bst_link[1] != NULL){
		traverse(node->bst_link[0], recommend, id);
		traverse(node->bst_link[1], recommend, id);
	}
}
...
void recommend_create(struct bst_table *, const string&);
...
int main()
{
	...
	int money;
	int time = 0, count = 0;
	struct bst_table *banksystem;
	banksystem = bst_create(account_cmp, NULL, NULL);

	while(cin >> command){
		if(command == "login"){
			cin >> ID1 >> psd1;
			temp.id = ID1;
			ptr = bst_find(banksystem, &temp);
			if(ptr == NULL)
			...
			if(ptr == NULL)
				...
				if(!temp.check_psd(md5(psd1)))
					...
					delete (Account*)bst_delete(banksystem, &temp);
					count--;
					cout << "success" << endl;
				}
			}
			...
			temp2.id = ID2;
			ptr = bst_find(banksystem, &temp);
			ptr2 = bst_find(banksystem, &temp2);
			if(ptr == NULL)
				cout << "ID " << ID1 << " not found" << endl;
			else if(ptr2 == NULL)
				...
				if(!m1->check_psd(md5(psd1)))
					...
					m1->merge_hist(m2);
					delete (Account*)bst_delete(banksystem, &temp2);
					count--;
				}
			}
		}
			...
			if(ptr == NULL){
				...
				traverse(banksystem->bst_root, recommend, ID1);
			...
			check_find = false;
			cin >> wildcard;
			isMatch(banksystem->bst_root, wildcard, curID);
			cout << endl;
		}
